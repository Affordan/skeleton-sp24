...
public class Model {
    ...
    public boolean atLeastOneMoveExists() {
        ...
        else                //the board is full
        {
            if (maxTileExists()) {
                ...
                // do not have the max tile . checkout all tile
                int size = board.size();
                for (int i = 0; i < size; i++) {
                    for (int j = 0; j < size; j++) {
                        if (i + 1 < size && board.tile(i, j).value() == board.tile(i + 1, j).value() ||
    ...
    public void moveTileUpAsFarAsPossible(int x, int y) {
        ...
        int size = board.size();
        while (targetY + 1 < size && board.tile(x, targetY + 1) == null) targetY++;

        if (targetY + 1 < size && board.tile(x, targetY + 1).value() == myValue) {
            Tile nextTile = board.tile(x, targetY + 1);
            if (nextTile != null && !nextTile.wasMerged()) {
